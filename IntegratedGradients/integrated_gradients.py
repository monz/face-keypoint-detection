...
def random_baseline_integrated_gradients(
  ...
  for i in range(num_random_trials):
    intgrads, prediction_trend = integrated_gradients(
       ...
       # baseline=np.zeros([28,28,1]),  # black baseline, based on float input [0,1]
       # baseline=np.random.random([28,28,1]),  # random noise baseline, based on float input [0,1]
       # baseline=255.0*np.random.random([28,28,1]),  # random noise baseline, based on float input [0,255]
       # baseline=255.0*np.random.random([96,96,3]),  # random noise baseline, based on uint8 input [0,255]
       # baseline=np.random.random([96,96,3]),  # random noise baseline, based on float input [0,1]
       # baseline=np.zeros([96,96,3]),  # black baseline, rgb
       baseline=np.zeros([96,96,1]),  # black baseline, gray-scale
       # baseline=np.random.random([96,96,1]),  # black baseline, gray-scale
       steps=steps)
